...
public class Client {
	...
	public static void main(String[] args) throws IOException {
		...
		// port
		SERVER_IP = args[0];
		SERVER_EXECUTE_PORT = Integer.parseInt(args[1]);
		SERVER_TERMINATE_PORT = Integer.parseInt(args[2]);

		
		
		Socket exec_socket = new Socket(SERVER_IP, SERVER_EXECUTE_PORT);
		Socket term_socket = new Socket(SERVER_IP, SERVER_TERMINATE_PORT);
		...
		PrintWriter out = new PrintWriter(exec_socket.getOutputStream(), true);
		PrintWriter out_term = new PrintWriter(term_socket.getOutputStream(), true);
